...
class SBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, downsample=False, ks=3, stride=1, upsample=False, add_coords=False):
        ...
        self.c1 = nn.Sequential(nn.Conv3d(in_planes, planes, kernel_size=ks, stride=stride, padding=pad),
...
class VAE(nn.Module):
    ...
    def __init__(self, input_size, sd=16, z_dim=20, out_chans=1, in_chans=1):
        ...
        self.decoder_inference.append(nn.Sequential(nn.Linear(self.half_z, self.dense_dims), nn.GELU()))
        self.decoder_reconstruction.append(nn.Sequential(nn.Linear(self.half_z, self.dense_dims), nn.GELU()))
        dec_sd = enc_sd
        for l in range(self.num_layers):
    ...
    def forward(self, x, y):
        ...
        log_var = torch.clamp(log_var, -20, 10) # clamp log-variance for numerical stability
...
class ModelWrapper(nn.Module):
    ...
    def forward(self, x, y, val=False, provided_mask=None, provided_scale=None, t=0.5, calibrate=False, lesion_threshold=None):
        ...
        recons = torch.sigmoid(recons) # comment this line if nn.BCEWithLogitsLoss is used.
        ...
        '''
        recon_ll = torch.sum(mse_fn(recons, x), dim=(-3, -2, -1)).mean()
